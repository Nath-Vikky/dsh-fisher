...
export class SaveStore {
  ...
  async load(): Promise<Save> {
    ...
    try {
      ...
      const backupName=sourceVersion===1||sourceVersion===2||sourceVersion===3||sourceVersion===4||sourceVersion===5||sourceVersion===6||sourceVersion===7||sourceVersion===8||sourceVersion===9?`save.before-v${sourceVersion+1}.json`
  ...
  async reset(save:Save):Promise<void> {
    ...
    try {
      ...
      const owned=/^save(?:\.backup|\.before-v(?:[2-9]|10)|\.before-content[34]|\.before-restore\.[12])?\.json(?:\.[0-9a-f-]{36}\.tmp)?$/;
