...
export function coastWater(pond=false,map:CoastMap=COASTS.L01):ShaderMaterial{
  ...
  return new ShaderMaterial({side:DoubleSide,defines:pond?{POND_WATER:1}:{},uniforms:{time:{value:0},pond:{value:pond?1:0},shallow:{value:new Color(pond?map.water.pond:map.water.shallow)},deep:{value:new Color(map.water.deep)},horizon:{value:new Color(map.light.fog)},poolCenter:{value:new Vector2(pool.x,pool.z)},poolSize:{value:new Vector2(pool.rx,pool.rz)},shoreCount:{value:map.coastline.length},shorePoints:{value:Array.from({length:16},(_,i)=>new Vector2(...(map.coastline[i]??map.coastline[0]!)))}},
    ...
    fragmentShader:`uniform float time;uniform float pond;uniform vec3 shallow;uniform vec3 deep;uniform vec3 horizon;uniform vec2 poolCenter;uniform vec2 poolSize;uniform vec2 shorePoints[16];uniform int shoreCount;varying vec3 shorePosition;
      void main(){vec2 p=shorePosition.xz;
        ...
        for(int i=0;i<16;i++){if(i>=shoreCount)break;vec2 a=shorePoints[i],b=shorePoints[i+1==shoreCount?0:i+1];vec2 edge=b-a;float along=clamp(dot(p-a,edge)/max(dot(edge,edge),.001),0.,1.);vec2 delta=p-a-edge*along;distanceSquared=min(distanceSquared,dot(delta,delta));}
        distanceToShore=sqrt(distanceSquared);
        #endif
