...
export function createCatalogView(React:typeof ReactTypes,FishArt:Art) {
  return function Catalog({data,controller,blocked,onFish}:{data:Bootstrap;controller:GameController;blocked:boolean;onFish:()=>void}) {
    ...
    return <section className="dsh-fisher-collection" aria-label="海岸图鉴"><div className="dsh-fisher-collection-intro"><h3>水边的相遇</h3><p>发现会留下。出售或放流，也不影响图鉴。</p>
      ...
      <input aria-label="搜索图鉴" placeholder="搜索已发现的名字" value={search} onChange={event=>setSearch(event.target.value)}/></div>
      {!entries.length&&<p>这一页暂时没有符合筛选的相遇。</p>}
      <div className="dsh-fisher-catalog-grid">{entries.map(entry=>{
        ...
        return <article className={`dsh-fisher-catalog-item${opened?' is-open':''}`} key={entry.id}>
          <button className="dsh-fisher-catalog-open" aria-expanded={opened} onClick={()=>{setSelected(opened?null:entry.id);setVariant('original');}}>
          ...
          {opened&&<div className="dsh-fisher-catalog-detail">
