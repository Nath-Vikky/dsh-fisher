...
export function createStorageView(React:typeof ReactTypes) {
  ...
  return function Storage({data,controller,busy}:{data:Bootstrap;controller:GameController;busy:boolean}) {
    ...
    return <fieldset className="dsh-fisher-storage"><legend>本机存档</legend>
      <p>把海岸带走，或从已有的文件恢复。导入会先显示内容预览。</p>
      ...
      <p>替换前会保留当前文件的恢复副本。导入后暂停原来的竿，关闭工作补给，已有储备仍保留。</p>
      ...
      {preview&&<Dialog title={preview.source==='backup'?'恢复上次备份':'导入存档预览'} busy={disabled} returnFocus={returnFocus.current} onClose={()=>setPreview(null)}>
      ...
      {deleting&&<Dialog title="删除海岸进度" busy={disabled} onClose={()=>setDeleting(false)}>
