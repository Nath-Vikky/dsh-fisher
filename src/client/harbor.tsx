...
import { BAITS, TIDES, TIDE_NAMES, bait, currentTide, levelInfo, regionUnlocked } from '../game/progression.ts';
    ...
    return <section className="dsh-fisher-collection dsh-fisher-harbor" aria-label="码头整备">
      ...
      {section&&<Dialog title={names[section]} busy={controller.getSnapshot().busy} error={controller.getSnapshot().error} onRetry={()=>void controller.retry()} onClose={()=>setSection(null)} className="dsh-fisher-harbor dsh-fisher-detail-window">
        ...
        {section==='bait'&&<>
          ...
          {!baitShop?<>
            <div className="dsh-fisher-bait-options">{BAITS.slice(page*4,page*4+4).map(item=><button key={item.id} aria-pressed={journey.bait===item.id} disabled={blocked||!idle||!preferenceAvailable(journey.region,item.id)||(item.id==='B08'&&!journey.invitations.length)||(item.id==='B07'&&(!clues||!missing.length))} onClick={()=>void controller.action({type:'bait.select',bait:item.id})}>{BAIT_ART[item.id]&&<img src={`${API}/assets/${thumbnailAsset(BAIT_ART[item.id]!)}`} alt=""/>}<span><strong>{item.name}</strong><small>{!preferenceAvailable(journey.region,item.id)?'本岸不可用':item.id==='B07'&&(!clues||!missing.length)?(!clues?'本岸完成10竿解锁':'本岸图鉴已集齐'):item.id==='B01'?'免费 · 无限':`剩余 ${item.id==='B08'?journey.invitations.length:journey.baits[item.id]??0} 份`}{journey.bait===item.id?' · 已选':''}</small></span></button>)}</div><Pager page={page} count={Math.ceil(BAITS.length/4)} onChange={setPage}/>
            <div className="dsh-fisher-review-card"><strong>当前：{bait(journey.bait).name}</strong><small>{bait(journey.bait).description}</small></div>
            {!idle&&<small>处理完这一竿，再更换鱼饵。</small>}
        ...
        {section==='gear'&&<><nav className="dsh-fisher-subtabs" aria-label="鱼具分类">{(['rod','line','float'] as const).map(id=><button key={id} aria-pressed={slot===id} onClick={()=>{setSlot(id);setPage(0);}}>{id==='rod'?'鱼竿':id==='line'?'鱼线':'浮漂'}</button>)}</nav>{gearItems.slice(page*3,page*3+3).map(item=>{const owned=journey.ownedGear.includes(item.id),equipped=journey.loadout[slot]===item.id;return <div key={item.id} className="dsh-fisher-shop-row"><img className="dsh-fisher-gear-art" src={`${API}/assets/${thumbnailAsset(GEAR_ART[item.id]!)}`} alt={item.name}/><div><b>{item.name}</b><small>{item.description}</small><small>{owned?'已拥有':`${item.price} 壳币 · Lv.${item.level}`}</small></div><button disabled={blocked||equipped||(owned?!idle:level.level<item.level||data.coins<item.price)} onClick={()=>void controller.action({type:owned?'gear.equip':'gear.buy',gear:item.id})}>{equipped?'已装备':owned?'装备':'购买'}</button></div>})}<Pager page={page} count={Math.ceil(gearItems.length/3)} onChange={setPage}/></>}
