...
当前分支版本为 `0.2.0-preview.1`，提供可走动的 3D 摸鱼塘预览，可从源码构建并本地安装，尚未发布到 npm。原像素版 `0.1.0-rc.11` 保留在 [archive/pixel-v1-rc11](https://github.com/Nath-Vikky/dsh-fisher/tree/archive/pixel-v1-rc11)。
...
- 细墨线漫画界面，保留像素图鉴与收获卡；其余三个地区仍使用原像素场景。支持浅深主题、字号、减少动态、低性能设置及默认关闭的轻声提示。
...
点击“摸鱼海岸”入口，在摸鱼塘拖动摇杆走向水边，点击“在这里钓鱼”，再点击“抛竿”。木栈桥和浅湾共用摸鱼塘的鱼池与装备。方向键／WASD 在聚焦海岸场景或摇杆时可移动人物；点击导航按钮可以自动走到钓点或来客身旁。其他地区沿用直接抛竿操作。
...
在钓鱼页勾选“自动钓鱼”，海岸会沿用当前钓点、鱼饵和装备，随 DSH 的模型响应与工具活动积累时间。基础等待按稀有度为 1、2、4、8 分钟有效活动，珠光、星砂及巨物需要更久。收获自动进入背包或图鉴，返回海岸时可查看汇总；背包满或缺饵时暂停。点击“接管这一竿”会关闭自动模式，保留同一份产物，并将等待比例转为收线进度，最后一段由你完成。
...
$fisherPackage = Join-Path $PWD 'nath-vikky-dsh-fisher-0.2.0-preview.1.tgz'
...
3D 当前是一座海岸的预览，人物与场景造型仍会调整。3D 模块首次打开时按需加载，并在加载提示后准备着色器和首帧；默认最高 30 FPS，低性能模式最高 20 FPS 并降低渲染分辨率。页面不可见或打开详情弹窗时暂停渲染，离开场景时释放图形资源。WebGL 不可用时可重试或切回轻量像素画面；实际流畅度仍取决于设备和宿主负载。
