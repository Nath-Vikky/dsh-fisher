...
3D 当前是一座海岸的预览，人物与场景造型仍会调整。3D 模块首次打开时按需加载，并在加载提示期间解码人物贴图、上传纹理、准备着色器和首帧；默认最高 30 FPS，低性能模式最高 20 FPS、降低渲染分辨率并关闭实时阴影。页面不可见或打开详情弹窗时暂停渲染，离开场景时释放图形资源。WebGL 不可用时可重试或切回轻量像素画面；实际流畅度仍取决于设备和宿主负载。
