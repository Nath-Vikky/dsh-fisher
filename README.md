...
当前版本为 `0.1.0-dev.6` 开发预览，可从源码构建并本地安装，尚未发布到 npm。支持钓鱼、收获处理、地区成长、鱼具整备、委托、来客与可选工作补给；部分插图和后续功能仍在制作。
...
- 工作补给默认关闭，启用后从 DSH 新活动获得额外补给，可自选随包鱼饵。
- 手记提供三份长期委托、24 项成就和四位来客的邀请、请求与相遇故事。
- 鱼缸可展示八个生物，陈列架可展示六件奇物或遗物；可购买和布置码头装饰。
- 背包可锁定留念，批量处理会保留新发现、首次外观、纪录、锁定和展示中的个体。
...
“码头”中的工作补给只观察启用后的回合边界和事件到达，不访问消息正文、工具参数或 Token 数量。多个会话同时工作合并计时；活动时长为保守估计，离线时间不累计。关闭游戏窗口仍可获得补给，关闭开关或禁用插件后停止观察。未领取补给可保留，领取支持重试。

“手记”中的委托在接下后记录新进度，交付订单可使用已有库存，需选定具体个体。展示中的收藏需先取回再交付。来客邀请可通过永久请求获得；认识后可免费重访，后续请求解锁故事和衣装。

奇珍异兽的名称与插图仍在更新，部分条目、衣装和装饰暂以文字呈现；收获卡导出及存档管理界面仍在制作。本版不是完整首发验收版。
...
```powershell
$env:DSH_HOME = Join-Path $PWD 'tmp/dsh-preview-dev6'
$fisherPackage = Join-Path $PWD 'nath-vikky-dsh-fisher-0.1.0-dev.6.tgz'
