...
当前版本为 `0.1.0-dev.5` 开发预览，可从源码构建并本地安装，尚未发布到 npm。支持钓鱼、收获处理、地区成长、鱼具整备与可选工作补给；部分插图和后续功能仍在制作。
...
当前预览尚不包含来客任务、委托成就和水族箱。奇珍异兽的名称与插图仍在更新，部分条目及外观暂以文字呈现；本版不是完整首发验收版。
...
```powershell
$env:DSH_HOME = Join-Path $PWD 'tmp/dsh-preview-dev5'
$fisherPackage = Join-Path $PWD 'nath-vikky-dsh-fisher-0.1.0-dev.5.tgz'
