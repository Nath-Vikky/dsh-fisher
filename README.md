...
- 四个钓点、48 个图鉴条目：28 种鱼、12 种奇珍异兽、4 件遗物和 4 位来客。36 种生物具有原色、珠光、星砂外观及尺寸纪录。
...
- 可拖动、缩放、停靠窗口，提供尺寸预设、宽高输入、比例锁定和位置恢复。
...
窗口偏好保存在当前浏览器。游戏进度保存在宿主 `$DSH_HOME/fishersave/`；未设置 `DSH_HOME` 时位于用户目录的 `.dsh/fishersave/`，独立于 Codekin。
